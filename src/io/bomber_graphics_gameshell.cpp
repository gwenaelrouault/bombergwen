...
void BomberGraphicsRenderer::draw(BomberTexture *texture, BomberCoordinates &coords)
{
    ...
    SDL_Rect box;
    box.x = round(screen_x1 * _x_scale_factor);
    box.y = round(screen_y1 * _y_scale_factor);
    box.w = round(texture->get_width() * _x_scale_factor);
    box.h = round(texture->get_height() * _y_scale_factor);
